...
const ChatComponent = () => {
  ...
  const saveMessages = (sessionId, messages) => {
    try {
      localStorage.setItem(`advisor_messages_${sessionId}`, JSON.stringify(messages));
    } catch (error) {
  ...
  const [messages, setMessages] = useState(() => {
    ...
    if (loaded.length === 0) {
      return [{
        role: 'assistant',
        content: "Hello! I'm your AI financial advisor, powered by advanced language models. I can help you with:\n\n• Stock analysis and recommendations\n• Portfolio management advice\n• Risk assessment\n• Investment strategy planning\n• Market insights\n\nHow can I assist you with your financial goals today?",
        timestamp: new Date()
      }];
    }
    return loaded;
  ...
  return (
    <div className="flex h-full bg-gradient-to-b from-white to-gray-50/50 dark:bg-black">
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gradient-to-b from-transparent to-gray-50/30 dark:bg-black">
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-200 dark:border-gray-800 p-5 bg-white/80 dark:bg-black/80 backdrop-blur-sm">
