...
import { 
  ArrowRight, ArrowLeft, Check, User, Briefcase, Sparkles, 
  Shield, Target, DollarSign, Building2, X, Plus, TrendingUp 
} from 'lucide-react';
...
const OnboardingWizard = ({ onComplete }) => {
  ...
  const handleNext = async () => {
    if (currentStep === 2) {
      ...
      try {
        ...
        alert(error.response?.data?.message || 'Failed to save portfolio. Please try again.');
      } finally {
        setLoading(false);
      }
  ...
  const handleAddHolding = () => {
    if (!holdingForm.symbol || !holdingForm.quantity || !holdingForm.averagePrice) {
                ...
                <div key={step} className="flex items-center flex-1">
                  ...
                  {step < totalSteps && (
                    <div className={`flex-1 h-1 mx-2 rounded ${
                      isCompleted ? 'bg-success-600' : 'bg-gray-300 dark:bg-gray-700'
                    }`} />
                  )}
                </div>
              );
          ...
          {currentStep === 1 && (
            <div className="text-center space-y-6">
              ...
              <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto leading-relaxed">
                Let's set up your personalized investment profile and portfolio. This will only take a few minutes,
            ...
            <div className="space-y-8">
              ...
              <div className="flex items-center gap-4 p-4 bg-gradient-to-br from-gray-50 to-gray-100 dark:bg-black rounded-xl border border-gray-200 dark:border-gray-600">
                <input
                  ...
                  onChange={(e) => setProfileData({ ...profileData, ethicalInvesting: e.target.checked })}
          ...
          {currentStep === 3 && (
            <div className="space-y-8">
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Add your current stock holdings (optional - you can add more later)
                </p>
              </div>
              ...
              {holdings.length > 0 && (
                <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Your Holdings</h3>
                    ...
                    {holdings.map((holding, index) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-gray-900 dark:text-white">{holding.symbol}</p>
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              {holding.quantity} shares @ ${parseFloat(holding.averagePrice).toFixed(2)}
                            </p>
                          </div>
                        </div>
                        <button
              ...
              {holdings.length === 0 && (
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <p>No holdings added yet. You can add them later from the Portfolio tab.</p>
                </div>
              )}
            </div>
          )}
